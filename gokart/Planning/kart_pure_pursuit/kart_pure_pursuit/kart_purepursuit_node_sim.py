...
class PurePursuit_node(Node):
    def __init__(self):
        ...
        # self.gnss_sub = self.create_subscription(NavSatFix, self.get_parameter('gnss_topic').get_parameter_value().string_value, self.gnss_cb, 10)
        # self.imu_sub = self.create_subscription(Imu, self.get_parameter('gnss_topic').get_parameter_value().string_value, self.imu_cb, 10)
        ...
        #                                          self.odom_cb, 10)
        self.odom_sub = self.create_subscription(Odometry, self.get_parameter('odom_topic').get_parameter_value().string_value, 
                                                 self.odom_cb, 10)
    ...
    # def gnss_cb(self, pose_msg: PoseWithCovarianceStamped):
    ...
    def odom_cb(self, odom: Odometry):
        ...
        self.target_pub.publish(pub_target_point)
        target_v = v_array[i_interp]
        speed = target_v * self.vel_scale  * 1.0
        ...
        message = AckermannDriveStamped()
        message.drive.speed = speed
        message.drive.acceleration = 0.01
        message.drive.steering_angle = steer
