...
def generate_launch_description():
    ...
    if OPTIMIZE:
        FILENAME = 'gnss_waypoints_purepursuit_optimal_sim.yaml'
    else:
        FILENAME = 'gnss_waypoints_purepursuit_original_sim.yaml'

    config = os.path.join(cwd, "src", "universe", "autoware.universe", "gokart", "configs", LOCATION, FILENAME)
    print(f"load config from {config}")

    purepursuit_node = Node(
        package="kart_pure_pursuit",
        executable="kart_purepursuit_node_sim",
        output="screen",
        parameters=[config],
    )
